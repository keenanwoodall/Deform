...
namespace DeformEditor
{
	...
	public class ElasticDeformableEditor : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			using (var check = new EditorGUI.ChangeCheckScope ())
			{
				...
				if (check.changed)
				{
					...
					foreach (var t in targets)
						((ElasticDeformable)t).UpdateMode = (UpdateMode)properties.UpdateMode.enumValueIndex;
				}
			}
			...
			if (newDeformers != null && newDeformers.Count > 0)
			{
				...
				foreach (var t in targets)
				{
					var elements = ((ElasticDeformable)t).DeformerElements;
			...
			using (new EditorGUILayout.HorizontalScope ())
			{
				...
				switch (selectedIndex)
				{
					...
					case 0:
						...
						foreach (var t in targets)
							((ElasticDeformable)t).DeformerElements.Clear ();
						break;
					case 1:
						Undo.RecordObjects (targets, "Cleaned Deformers");
						foreach (var t in targets)
							((ElasticDeformable)t).DeformerElements.RemoveAll (d => d.Component == null);
						break;
					case 2:
						foreach (var t in targets)
						{
							var deformable = t as ElasticDeformable;
			...
			if (foldoutDebug = EditorGUILayoutx.FoldoutHeader ("Debug Info", foldoutDebug))
			{
				...
				var bounds = (target as ElasticDeformable).GetMesh ().bounds;
				foreach (var t in targets)
				{
					var deformable = t as ElasticDeformable;
		...
		private void OnSceneGUI ()
		{
			if (foldoutDebug)
			{
				var deformable = target as ElasticDeformable;
		...
		{
			var deformable = (ElasticDeformable)command.context;
